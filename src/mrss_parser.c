...
static void __mrss_parse_tag_insert(mrss_tag_t **where, mrss_tag_t *what) {
  if (!*where)
    *where = what;
  else {
...
static mrss_tag_t *__mrss_parse_tag(nxml_t *doc, nxml_data_t *cur) {
  mrss_tag_t *tag;
  ...
  for (nxml_attr = cur->attributes; nxml_attr; nxml_attr = nxml_attr->next) {
    ...
    if (!(attribute = (mrss_attribute_t *)calloc(1, sizeof(mrss_attribute_t))))
      return NULL;
...
static void __mrss_parser_atom_string(nxml_t *doc, nxml_data_t *cur,
                                      char **what, char **type) {
  char *c;
...
static char *__mrss_atom_prepare_date(mrss_t *data, char *datestr) {
  struct tm stm;
...
static void __mrss_parser_atom_category(nxml_data_t *cur,
...
static void __mrss_parser_atom_author(nxml_data_t *cur, char **name,
                                      char **email, char **uri) {
  for (cur = cur->children; cur; cur = cur->next) {
...
static void __mrss_parser_atom_entry(nxml_t *doc, nxml_data_t *cur,
                                     mrss_t *data) {
  char *c;
  mrss_item_t *item;
  ...
  for (cur = cur->children; cur; cur = cur->next) {
    if (cur->type == NXML_TYPE_ELEMENT) {
      ...
      if (!item->title && !strcmp(cur->value, "title"))
        __mrss_parser_atom_string(doc, cur, &item->title, &item->title_type);
      ...
      /* author structure -> author elements */
        ...
        __mrss_parser_atom_author(cur, &item->author, &item->author_email,
                                  &item->author_uri);
...
static void __mrss_parser_rss_image(nxml_t *doc, nxml_data_t *cur,
                                    mrss_t *data) {
  char *c;
  ...
  for (cur = cur->children; cur; cur = cur->next) {
    if (cur->type == NXML_TYPE_ELEMENT) {
      ...
      /* description */
      else if (!strcmp(cur->value, "description") && !data->image_description &&
               (c = nxmle_get_string(cur, NULL)))
...
static void __mrss_parser_rss_textinput(nxml_t *doc, nxml_data_t *cur,
                                        mrss_t *data) {
  char *c;
...
static void __mrss_parser_rss_skipHours(nxml_t *doc, nxml_data_t *cur,
                                        mrss_t *data) {
  char *c;

  for (cur = cur->children; cur; cur = cur->next) {
    if (cur->type == NXML_TYPE_ELEMENT) {
      if (!strcmp(cur->value, "hour") && (c = nxmle_get_string(cur, NULL))) {
        mrss_hour_t *hour;
...
static void __mrss_parser_rss_skipDays(nxml_t *doc, nxml_data_t *cur,
                                       mrss_t *data) {
  char *c;

  for (cur = cur->children; cur; cur = cur->next) {
    if (cur->type == NXML_TYPE_ELEMENT) {
      if (!strcmp(cur->value, "day") && (c = nxmle_get_string(cur, NULL))) {
        mrss_day_t *day;
...
static void __mrss_parser_rss_item(nxml_t *doc, nxml_data_t *cur,
                                   mrss_t *data) {
  char *c;
  char *attr;
  mrss_item_t *item;
...
static mrss_error_t __mrss_parser_atom(nxml_t *doc, nxml_data_t *cur,
                                       mrss_t **ret) {
  mrss_t *data;
  char *c = NULL;
  ...
  for (cur = cur->children; cur; cur = cur->next) {
    if (cur->type == NXML_TYPE_ELEMENT) {
      ...
      if (!data->title && !strcmp(cur->value, "title"))
        __mrss_parser_atom_string(doc, cur, &data->title, &data->title_type);

      /* subtitle -> description */
      else if (!data->description && data->version == MRSS_VERSION_ATOM_1_0 &&
      ...
      else if (data->version == MRSS_VERSION_ATOM_0_3 && !data->description &&
               !strcmp(cur->value, "tagline"))
        __mrss_parser_atom_string(doc, cur, &data->description,
                                  &data->description_type);
...
static mrss_error_t __mrss_parser_rss(mrss_version_t v, nxml_t *doc,
                                      nxml_data_t *cur, mrss_t **ret) {
  mrss_t *data;
  char *c, *attr;
  ...
  for (cur = cur->children; cur; cur = cur->next) {
    if (cur->type == NXML_TYPE_ELEMENT) {
      ...
      /* lastBuildDate */
      else if (!strcmp(cur->value, "lastBuildDate") && !data->lastBuildDate &&
               (c = nxmle_get_string(cur, NULL)))
      ...
      /* managingeditor */
      else if (!strcmp(cur->value, "managingeditor") && !data->managingeditor &&
               (c = nxmle_get_string(cur, NULL)))
...
static mrss_error_t __mrss_parser(nxml_t *doc, mrss_t **ret) {
  ...
  if (!strcmp(cur->value, "rss")) {
    if ((c = nxmle_find_attribute(cur, "version", NULL))) {
      /* 0.91 VERSION */
      if (!strcmp(c, "0.91"))
        r = __mrss_parser_rss(MRSS_VERSION_0_91, doc, cur->children, ret);

      /* 0.92 VERSION */
      else if (!strcmp(c, "0.92"))
        r = __mrss_parser_rss(MRSS_VERSION_0_92, doc, cur->children, ret);
...
mrss_error_t mrss_parse_url(char *url, mrss_t **ret) {
  return mrss_parse_url_with_options_error_and_transfer_buffer(
      url, ret, NULL, NULL, NULL, NULL);
}

mrss_error_t mrss_parse_url_with_options(char *url, mrss_t **ret,
                                         mrss_options_t *options) {
  return mrss_parse_url_with_options_error_and_transfer_buffer(
      url, ret, options, NULL, NULL, NULL);
}

mrss_error_t mrss_parse_url_with_options_and_error(char *url, mrss_t **ret,
                                                   mrss_options_t *options,
                                                   CURLcode *code) {
  return mrss_parse_url_with_options_error_and_transfer_buffer(
      url, ret, options, code, NULL, NULL);
}

mrss_error_t mrss_parse_url_with_options_error_and_transfer_buffer(
    char *url, mrss_t **ret, mrss_options_t *options, CURLcode *code,
    char **feed_content, int *feed_size) {
  nxml_t *doc;
  mrss_error_t err;
  char *buffer;
  size_t size;
